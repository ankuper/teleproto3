/*
 * test_csprng_windows.c — Statistical smoke test for the Windows CSPRNG backend.
 *
 ...
 *
 * Story 1-14 (AC #2). Mirrors test_csprng_linux.c and test_csprng_macos.c.
 */
...
int main(void) {
    uint8_t *buf = calloc(1, SAMPLE_BYTES);
    if (!buf) {
        fprintf(stderr, "test_csprng_windows: calloc failed\n");
        return 1;
    }

    t3_result_t rc = t3_csprng_bytes(buf, SAMPLE_BYTES);
    if (rc != T3_OK) {
        fprintf(stderr, "test_csprng_windows: t3_csprng_bytes returned %s\n",
                t3_strerror(rc));
        free(buf);
        return 1;
    }
