/*
 * test_csprng_macos.c — Statistical smoke test for the macOS CSPRNG backend.
 *
 ...
 *
 * Story 1-13 (AC #2). Mirrors test_csprng_linux.c exactly.
 */
...
int main(void) {
    ...
    if (!buf) {
        fprintf(stderr, "test_csprng_macos: calloc failed\n");
        return 1;
    }

    t3_result_t rc = t3_csprng_bytes(buf, SAMPLE_BYTES);
    if (rc != T3_OK) {
        fprintf(stderr, "test_csprng_macos: t3_csprng_bytes returned %s\n",
                t3_strerror(rc));
        free(buf);
        return 1;
    }
